...
def color_masking(img,color):
    ...
    elif color == 'yellow_back':
        lower = np.array([80, 130, 70])
        upper = np.array([120, 220, 255])
    elif color == 'green':
...
def lines_finder(img, color, position):  
    ...
    # if position == 'G_right' and color == 'green':
    #     cv2.imwrite('right_mask.jpg', mask)
    #     import time; time.sleep(0.3)     
    # if position == 'G_left' and color == 'green':
...
def slope_select(points, slopes, lengths, hope_angle, position, len_threshold):
    ...
    elif hope_angle == 95: angle_range = (60, -60)
    ...
    for _ in range(len(lengths)):
        ...
        if (angle_range[0] < slope_selected or slope_selected < angle_range[1]) and lengths[idx] > len_threshold : 
            # if position == 'left' or position == 'right': rospy.loginfo(position + ': points: {} | slope: {} | length: {}'.format(points[idx],slopes[idx], lengths[idx]))
...
def detect_obstacle_pt(img, color, position, y_min:bool):
    ...
    try: 
        ...
        min_value = None
        min_point = None
    # if position == 'back' and color == 'blue':
    #     pass
    #     cv2.imwrite('back_blue.jpg', mask_img)
    #     import time; time.sleep(1)    
    # if position == 'front' and color == 'yellow':
    #     pass
    #     cv2.imwrite('front_yellow.jpg', mask_img)
    #     import time; time.sleep(0.3)       
