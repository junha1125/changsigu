...
class image_converter:
  def __init__(self):
    ...
    self.detec_count = 0
    self.Obs = []
    self.Obs_y = self.Start_y + 2.53 + 1.28
  ...
  def slope_select(self, slope, length, direction):
    if direction=='right' or direction =='left':
	  ...
	  if 75 < slope_selected or slope_selected < -75: 
	    slope_selected = None
          return slope_selected
        else:
          return None
    else:
        return self.avg_slope(slope)
  ...
  def main_calculator(self):
    ...
    now = datetime.now()
    cv2.imwrite('bev_image_{}.jpg'.format(now.strftime("%M_%S")), bev_image)
    ...
    contour_image, min_value = detect_obstacle_pt(img_pered_front, 'yellow')
    # front_corner, front_w_corner_vis, mask_img = detect_obstacle_pt(img_pered_front, 'yellow')
  
    
    # cv2.imwrite('image_contour.jpg',contour_image)
    # # cv2.imwrite('mask_img.jpg',mask_img)
    # # # print('mask: ', mask)
    # # # print('mask_max: ', mask.max())
    # # # print('mask_min: ', mask.min())
    # # print('point number:', len(front_corner))
    # # print('front_corner:', front_corner)
    # print('left_slope:', slopes_left_avg)

    return slopes_front_avg, slopes_back_avg, slopes_right_avg, slopes_left_avg, min_value
  ...
  def callback3(self,data):
    try:
      ...
      if all([self.img0_get_bool,self.img1_get_bool,self.img2_get_bool,self.img3_get_bool]) == True:
        slopes_front_avg, slopes_back_avg, slopes_right_avg, slopes_left_avg, min_value = self.main_calculator()
  ...
  def callback4(self,data):
    try:
      ...
      # print(self.img0_get_bool,self.img1_get_bool,self.img2_get_bool,self.img3_get_bool)
      # if all([self.img0_get_bool,self.img1_get_bool,self.img2_get_bool,self.img3_get_bool]) == True:
      #   slopes_front_avg, slopes_back_avg, slopes_right_avg, slopes_left_avg, min_value = self.main_calculator()
  ...
  def Constraint(self):
      ...
      if self.Obs_y > self.tolerance2:
        self.stop = True
        print('Stop due to min value')  

      # elif self.Obs[1] - self.y < self.tolerance:
      #   print('Stop due to Ds')
      #   v = self.Stop()
  ...
  def PathTracking(self,ds,vision_yaw):
      ...
      if self.Camera_Obs is not None:
        ...
        self.detec_count = self.detec_count + 1
